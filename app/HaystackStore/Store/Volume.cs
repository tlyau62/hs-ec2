...
public class Volume : IVolume, IDisposable
{
    ...
    public byte[] ReadData(Metadata metadata)
    {
        var bytes = new byte[metadata.Size];
        var offset = metadata.Offset + sizeof(uint) + sizeof(long) + sizeof(byte) + sizeof(int); // needle offset + header + key + flags + size
        RandomAccess.Read(_fileHandle, bytes, offset);

        _fileWait.WaitBytesRead(metadata.Size);
